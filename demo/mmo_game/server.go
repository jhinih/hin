...
import (
	"fmt"
	"github.com/jhinih/hin/demo/mmo_game/api"
...
// 当客户端建立连接的时候的hook函数
func OnConnectionAdd(conn hinterface.IConnection) {
	fmt.Println("=====> OnConnectionAdd is Called ...")

	//AI
	AIplayer := core.NewPlayer(conn)
	AIplayer.SyncPID()
	AIplayer.BroadCastStartPosition()
	core.WorldMgrObj.AddPlayer(AIplayer)
	conn.SetProperty("AIpID", AIplayer.PID)
	AIplayer.SyncSurrounding()
	fmt.Println("=====> AIPlayer pIDID = ", AIplayer.PID, " arrived ====")
...
// 当客户端断开连接的时候的hook函数
func OnConnectionLost(conn hinterface.IConnection) {

	//AI
	AIpID, _ := conn.GetProperty("AIpID")
	var AIplayerID int32
	AIplayer := core.WorldMgrObj.GetPlayerByPID(AIplayerID)
	if AIpID != nil {
		AIplayerID = AIpID.(int32)
	}
	if AIplayer != nil {
		AIplayer.LostConnection()
	}
	fmt.Println("====> AIPlayer ", AIplayerID, " left =====")
...
func main() {
	...
	// 注册客户端连接建立和丢失函数
	s.SetConnectionStartHook(OnConnectionAdd)
	s.SetConnectionStopHook(OnConnectionLost)
