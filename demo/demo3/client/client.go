...
func ClientConnectionStartHook(connection hinterface.IConnection) {
	zlog.Infof("%d", 1)
	zlog.Infof("=====》[Client connection %s start]", strconv.Itoa(int(connection.GetConnectionID())))
	connection.SetProperty("web", "hin")
	for {
		if err := connection.Send(0, []byte("ClientConnectionStartHook send : {huh huh}")); err != nil {
...
func ClientConnectionStopHook(connection hinterface.IConnection) {
	...
	zlog.Infof("ClientConnectionStopHook %s", connection.GetProperty("web"))
